...
from MultyGraphModel import BiGraphAutoEncoder, discret_tensor
from Utils import crate_anndata, save_model, save_obj, build_network
...
def train(data, loader ,highly_variable_index,number_of_batches=5 , max_epoch = 500, rduce_interavel = 50,writer = None,model_name="", train_all = False):
    x = data.x.clone()
    model = BiGraphAutoEncoder(x.shape[0], x.shape[1]//number_of_batches, 250, 75, 250, 75, lambda_rows = 1, lambda_cols=1, vae_flag=False, add_linear_row=False, add_linear_col=False, num_layers=2, cd=False) .to(device)
    ...
    for epoch in range(max_epoch):
        ...
        for _,batch in enumerate(loader):
            ...
            #isin_result  = torch.isin(loader.dataset.edge_index, idx.to(device))
            #columns_to_select = isin_result.all(dim=0)
            #knn_edge_index = loader.dataset.edge_index[:,columns_to_select]
            #knn_edge_index = knn_edge_index.cpu().apply_(lambda x:torch.where(idx.cpu() == x)[0]).to(device)
            #if knn_edge_index.shape[1] == knn_loader.dataset.edge_index.shape[0] // number_of_batches:
            if x.shape[1] == data.x.shape[1] // number_of_batches:
        ...
        if (epoch+1) % rduce_interavel == 0 and col_flag:
            print(new_knn_edge_index.shape[1] / knn_loader.dataset.edge_index.shape[0])
            knn_loader = mini_batch_knn(new_knn_edge_index, new_knn_edge_index.shape[1] // number_of_batches)
           # optimizer.param_groups[0]["lr"] *= 0.9
            if (torch.unique(new_knn_edge_index[0]).shape[0] < x.shape[1]):
                flag = 1
        ...
        if epoch%10 == 0:
          ...
          if auc > best_auc - 0.01 and train_all:
            ...
            #save_obj(new_knn_edge_index, "Best_new_knn_graph")
    ...
    if train_all:
         ...
         return model, new_knn_edge_index
...
def train_on_k_fold(folds, knn_loader ,highly_variable_index,number_of_batches=5, max_epoch = 600, rduce_interavel = 50, data=None,writer = None):
    fpr_list = []
    tpr_list = []
    auc_list = []
    mean_fpr = np.linspace(0, 1, 1000)
    for data in folds:
        model,knn_edge_index = train(data, knn_loader ,highly_variable_index, number_of_batches=number_of_batches, max_epoch=max_epoch, rduce_interavel=30, writer=writer, train_all= False)
        model.eval()   
        with torch.no_grad():
            embbed = model.encoder(data.x, knn_edge_index, data.train_pos_edge_index)
...
def crate_k_fold_cross_validation(x, ppi_edge_index, k=3):
...
def main(path = "./Data/cell_line.h5ad",pre_processing_flag = True ,biogrid_flag = False, train_all = False, human_flag=False, random_noise=False, model_name=""):
    ...
    number_of_batches = 5
    knn_loader = mini_batch_knn(knn_edge_index, knn_edge_index.shape[1] // number_of_batches)
   ...
   # x = discret_tensor(x)
    cell_loader = mini_batch_cells(x, knn_edge_index, x.shape[1] // number_of_batches)

    if random_noise:
  #    node_feature = node_feature.sample(frac=1)
      x += torch.tensor(np.random.normal(loc=0,scale=5,size=x.shape), device=device)

    if not train_all:
        folds =  crate_k_fold_cross_validation(x, ppi_edge_index)
        train_on_k_fold(folds, knn_loader, highly_variable_index,max_epoch=100, rduce_interavel=30, number_of_batches=number_of_batches)
    else:
        writer = SummaryWriter(log_dir = f"./runs")  
        data = Data(x,ppi_edge_index)
        data = train_test_split_edges (data,test_ratio=0.2, val_ratio=0)
        model = train(data, cell_loader ,highly_variable_index, number_of_batches=number_of_batches, max_epoch=539, rduce_interavel=30, writer=writer,model_name=model_name, train_all= True)
        writer.flush()
...
if __name__ == "__main__":
    main(path = r"./Data/cell_line.h5ad", pre_processing_flag = False,human_flag=True, train_all = True,random_noise=False)